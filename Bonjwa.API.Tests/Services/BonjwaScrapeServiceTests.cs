...
namespace Bonjwa.API.Tests.Services
{
    public class BonjwaScrapeServiceTest
    {
        ...
        public async Task TestScrapeAsync(string html)
        {
            ...
            mockFetcher.Setup(x => x.FetchAsync(It.IsAny<string>()).Result).Returns(html);

            var service = new BonjwaScrapeService(mockLogger.Object, mockFetcher.Object);

            var (eventItems, scheduleItems) = await service.ScrapeEventsAndScheduleAsync();
            ...
            Assert.Equal(DateTime.Parse("2021-07-26T16:00:00.0000000+02:00"), firstItem.StartDate);
            Assert.Equal(DateTime.Parse("2021-07-26T18:00:00.0000000+02:00"), firstItem.EndDate);
            ...
            Assert.Equal(DateTime.Parse("2021-07-29T00:00:00.0000000+02:00"), lastItem.StartDate);
            Assert.Equal(DateTime.Parse("2021-07-29T03:00:00.0000000+02:00"), lastItem.EndDate);
            Assert.False(firstItem.Cancelled);
